...
import DeleteChoice from 'features/offers/choices/components/DeleteChoice';
...
function ChoiceOverview() {
  const choicesArray = useSelector((state) => {
    let choicesArray = Object.values(state.offers.choices.byId);
    choicesArray.sort((a, b) => a.name.localeCompare(b.name));
    return choicesArray;
  });
  const selectedChoiceId = useSelector((state) => state.offers.views.itemId);
