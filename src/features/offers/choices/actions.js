...
export const createChoice = createAsyncThunk('choices/createChoice', async (data, thunkAPI) => {
  ...
  const fetchParams = createFetchParams('owner/offers/choices', 'POST', { dishes, choice });
...
export const updateChoice = createAsyncThunk('dishes/updateChoice', async (data, thunkAPI) => {
  let { dishes, updated, created, ...choice } = data;

  const fetchParams = createFetchParams('owner/offers/choices', 'PUT', { dishes, choice });
  const response = await fetch(fetchParams.url.href, fetchParams.options);
  if (response.ok) {
    const data = await response.json();
    return Promise.resolve(data);
...
export const deleteChoice = createAsyncThunk('choices/deleteChoice', async (id, thunkAPI) => {
  const path = `owner/offers/choices/${id}`;
...
export const setSubs = createAsyncThunk('choices/setSubs', async (data, thunkAPI) => {
  const fetchParams = createFetchParams('owner/offers/choices/subs', 'PUT', data);
  const response = await fetch(fetchParams.url.href, fetchParams.options);
  if (response.ok) {
    return Promise.resolve(data);
  } else {
...
export const removeSub = createAsyncThunk('choices/removeSub', async (data, thunkAPI) => {
  const path = `owner/offers/choices/${data.choiceId}/subs/${data.subId}`;
  const fetchParams = createFetchParams(path, 'DELETE');
