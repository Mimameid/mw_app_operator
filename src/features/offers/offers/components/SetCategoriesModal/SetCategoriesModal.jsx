...
import { setCategories } from 'features/offers/offers/actions';
import { selectCategoriesAsArray } from 'features/offers/categories/slice';
...
function SetCategoriesModal({ open, setOpen, offer }) {
  const dispatch = useDispatch();
  const categoriesArray = useSelector(selectCategoriesAsArray);

  const { handleSubmit, control, reset } = useForm({
    mode: 'onChange',
    defaultValues: { categories: offer.categories },
  ...
  useEffect(() => {
    reset({ categories: offer.categories });
  }, [reset, offer]);
  ...
  async function onSubmit(data) {
    setLoading(true);
    await dispatch(setCategories({ offerId: offer.id, categories: data.categories }));
    setOpen(false);
  }
