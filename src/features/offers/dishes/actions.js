...
export const createDish = createAsyncThunk('dishes/createDish', async (data, thunkAPI) => {
  ...
  const fetchParams = createFetchParams('owner/offers/dishes', 'POST', { categories, dish });
...
export const updateDish = createAsyncThunk('dishes/updateDish', async (data, thunkAPI) => {
  ...
  const fetchParams = createFetchParams('owner/offers/dishes', 'PUT', { categories, dish });
  const response = await fetch(fetchParams.url.href, fetchParams.options);
  if (response.ok) {
    const data = await response.json();
    return Promise.resolve(data);
...
export const deleteDish = createAsyncThunk('dishes/deleteDish', async (id, thunkAPI) => {
  const path = `owner/offers/dishes/${id}`;
...
export const setChoices = createAsyncThunk('dishes/setChoices', async (data, thunkAPI) => {
  const fetchParams = createFetchParams('owner/offers/dishes/choices', 'PUT', data);
  const response = await fetch(fetchParams.url.href, fetchParams.options);
  if (response.ok) {
    return Promise.resolve(data);
  } else {
...
export const removeChoice = createAsyncThunk('dishes/removeChoice', async (data, thunkAPI) => {
  const path = `owner/offers/dishes/${data.dishId}/choices/${data.choiceId}`;
  const fetchParams = createFetchParams(path, 'DELETE');
...
export const setAvailable = createAsyncThunk('dishes/setAvailable', async (data, thunkAPI) => {
  const fetchParams = createFetchParams('owner/offers/dishes/available', 'PUT', data);
  const response = await fetch(fetchParams.url.href, fetchParams.options);
