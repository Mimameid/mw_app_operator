...
import { makeSelectAffectedOffers, selectOfferIdsToNames } from 'features/offers/offers/slice';
...
const defaultValues = { name: '', desc: '', offers: [] };

function CategoryModal({ open, onClose, category }) {
  const dispatch = useDispatch();
  const selectAffectedOffers = useMemo(makeSelectAffectedOffers, []);
  const affectedOffers = useSelector((state) => selectAffectedOffers(state, category ? category.id : null));
  const offerIdsToNames = useSelector(selectOfferIdsToNames);
  const [loading, setLoading] = useState(false);

  const { handleSubmit, control, reset, formState } = useForm({
    mode: 'onChange',
    defaultValues: category ? { name: category.name, desc: category.desc, offers: affectedOffers } : defaultValues,
    delayError: 300,
    resolver: yupResolver(schema),
  });

  const resetValues = useCallback(() => {
    reset(category ? { name: category.name, desc: category.desc, offers: affectedOffers } : defaultValues);
  }, [category, affectedOffers, reset]);

  const onSubmit = async (data) => {
    setLoading(true);
    data.offers = data.offers.map((item) => item[0]);
