...
import { setChoices } from 'features/products/products/actions';
import { selectChoicesAsArray } from 'features/products/choices/slice';
...
function SetChoicesModal({ open, setOpen, product }) {
  const dispatch = useDispatch();
  const choicesArray = useSelector(selectChoicesAsArray);

  const { handleSubmit, control, reset } = useForm({
    mode: 'onChange',
    defaultValues: { choices: product.choices },
  ...
  async function onSubmit(data) {
    setLoading(true);
    await dispatch(setChoices({ productId: product.id, choices: data.choices }));
    setOpen(false);
  }
